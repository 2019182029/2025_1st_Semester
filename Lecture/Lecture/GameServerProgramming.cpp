/*
 ...
 1. Boost
    → C++ 프로그래밍에 사용되는 라이브러리 집합
       많은 라이브러리를 헤더 파일로 구현 : 템플릿 프로그래밍
                                            → 장점 : 호환성, 실행 속도
                                               단점 : 가독성, 컴파일 속도, 일부는 바이너리 라이브러리 필요


 2. ASIO
    → 기존 소켓 API를 전부 C++ API로 재작성 : 직관적이어서 사용하기 편하다.
                                               람다 함수에 매우 의존적이다.
                                               소켓 객체의 관리에 주의해야 한다.
                                               → 소멸자가 호출되어야 커널 자료구조가 반환된다.
                                                  멀티쓰레드 사용 시 shared_ptr가 강제된다.
    ...
    → 멀티쓰레드 연동 : io_context에 대한 모든 접근은 쓰레드 safety를 보장한다.
                         → 여러 개의 쓰레드에서 io_context::run()을 호출함으로써 병렬성을 얻을 수 있다.
                            내부적으로 비동기 처리를 위한 별도의 쓰레드를 사용할 수 있는데, 이것의 동작은 최소화되고 사용자에게 숨겨진다.
