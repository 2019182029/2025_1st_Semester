...
public:
	...
	void wake_up() {
		...
		if (std::atomic_compare_exchange_strong(&_is_active, &expected, true)) {
			timer_lock.lock();
			timer_queue.emplace(event_type{ _id, chrono::high_resolution_clock::now() + chrono::seconds(1), EV_MOVE, 0 });
			timer_lock.unlock();
		}
	}
...
void worker_thread(HANDLE h_iocp)
{
	while (true) {
		...
		case OP_NPC_MOVE: {
			...
			std::shared_ptr<SESSION> npc = clients.at(cliend_id);
			if (nullptr == npc) return;

			do_npc_random_move(npc->_id);
			if (npc->_is_active) {
				timer_lock.lock();
				timer_queue.emplace(event_type{ npc->_id, chrono::high_resolution_clock::now() + chrono::seconds(1), EV_MOVE, 0 });
				timer_lock.unlock();
			}
			delete ex_over;
			break;
...
void InitializeNPC()
{
	...
	//cout << "NPC move begin.\n";
	//for (int i = MAX_USER; i < MAX_USER + MAX_NPC; ++i) {
	//	timer_queue.emplace(event_type{ i, chrono::high_resolution_clock::now() + chrono::seconds(1), EV_MOVE, 0 });
	//}
	//cout << "NPC move end.\n";
}
